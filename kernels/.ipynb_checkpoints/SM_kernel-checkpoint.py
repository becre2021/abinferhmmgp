...
class SM(StationaryKernel):
    ...
    def _assign_SM_param(self,weight,mu,std):
        ...
        if self.input_dim > 1:
            self.ARD = True
        self.weight = Param(torch.tensor(weight).to(self.device), requires_grad=True, requires_transform=True, param_name='sm_weight' )
        self.mu = Param(torch.tensor(mu).to(self.device) , requires_grad=True, requires_transform=True, param_name='sm_mu' )
        self.std = Param(.5*torch.tensor(std).to(self.device), requires_grad=True, requires_transform=True,param_name='sm_sigma')
        #self.std = Param(.2*torch.tensor(std).to(self.device), requires_grad=True, requires_transform=True,param_name='sm_sigma')
